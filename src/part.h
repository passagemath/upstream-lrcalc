...
INLINE int part_length(ivector *p)
{
  int len;
  len = iv_length(p);
  while (len > 0 && iv_elem(p, len-1) == 0)
    len--;
  return len;
}

INLINE int part_entry(ivector *p, int i)
{
  return (i < iv_length(p)) ? iv_elem(p,i) : 0;
}

INLINE void part_chop(ivector *p)
{
  iv_length(p) = part_length(p);
}
...
INLINE int part_leq(ivector *p1, ivector *p2)
{
  int i, len;
  len = part_length(p1);
  if (len > part_length(p2))
    return 0;
  for (i = len-1; i >= 0; i--)
    if (iv_elem(p1, i) > iv_elem(p2, i))
      return 0;
  return 1;
}
...
INLINE int pitr_good(part_iter *itr)
{
  return itr->rows >= 0;
}
