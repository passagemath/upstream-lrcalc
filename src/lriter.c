...
{
  lrtab_iter *lrit;
  lrit_box *box, *box_bound, *array;
  ivector *cont;
  int rows, x, max;
  long long coef;

  rows = part_length(content);
  lrit = lrit_new(outer, inner, NULL, rows, -1, rows);
  if (lrit == NULL)
    return -1;
  if (lrit->size <= 1)
    {
      coef = lrit_good(lrit) ? 1 : 0;
      lrit_free(lrit);
      return coef;
    }
  ...
  iv_set_zero(cont);
  array = lrit->array;
  box_bound = lrit->array + lrit->size;

  box = box_bound - 1;
  x = array[box->above].value;
  coef = 0;
  ...
  while (1)
    {
      ...
      x++;
      while (x <= max &&
             ((x && iv_elem(cont,x) == iv_elem(cont,x-1))
              || iv_elem(cont,x) == iv_elem(content,x)))
        x++;
      if (x > max)
        {
          box++;
