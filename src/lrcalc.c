...
usage_t mult_usage = {
 name: "mult",
 args: "[-m] [-r rows] [-c cols] [-q rows,cols] [-f rows,level] " \
 "part1 - part2"
};
...
void mult_main(int ac, char **av)
{
  ...
  while ((c = getopt(ac, av, "mr:c:q:f:")) != EOF)
    switch (c)
      {
      case 'm':
	opt_maple = 1;
  ...
  if (sh1 == NULL || part_valid(sh1) == 0)
    cmd_error(&mult_usage, "part1 not a valid partition.");
  sh2 = get_vect_arg(ac, av);
  if (sh2 == NULL || part_valid(sh2) == 0)
    cmd_error(&mult_usage, "part2 not a valid partition.");
...
void skew_main(int ac, char **av)
{
  ...
  if (outer == NULL || part_valid(outer) == 0)
    cmd_error(&skew_usage, "outer shape not a valid partition.");
  inner = get_vect_arg(ac, av);
  if (inner == NULL || part_valid(inner) == 0)
    cmd_error(&skew_usage, "inner shape not a valid partition.");
...
void coprod_main(int ac, char **av)
{
  ...
  if (sh == NULL || part_valid(sh) == 0)
    cmd_error(&mult_usage, "part not a valid partition.");
...
void coef_main(int ac, char **av)
{
  ...
  outer = get_vect_arg(ac, av);
  if (outer == NULL || part_valid(outer) == 0)
    cmd_error(&coef_usage, "outer not a valid partition.");
  sh1 = get_vect_arg(ac, av);
  if (sh1 == NULL || part_valid(sh1) == 0)
    cmd_error(&coef_usage, "inner1 not a valid partition.");
  sh2 = get_vect_arg(ac, av);
  if (sh2 == NULL || part_valid(sh2) == 0)
    cmd_error(&coef_usage, "inner2 not a valid partition.");
...
void tab_main(int ac, char **av)
{
  ...
  outer = get_vect_arg(ac, av);
  if (outer == NULL || part_valid(outer) == 0)
    cmd_error(&tab_usage, "outer shape not a valid partition.");
  inner = get_vect_arg(ac, av);
  if (inner == NULL || part_valid(inner) == 0)
...
usage_t *lrcalc_commands[] = {
  &mult_usage, &skew_usage, &coprod_usage, &coef_usage, &tab_usage, NULL};

void main_usage()
{
  usage_t **cmd;
  fprintf(stderr, "Usage:\n");
  for (cmd = lrcalc_commands; *cmd != NULL; cmd++)
    fprintf(stderr, PROGNAME " %s %s\n", (*cmd)->name, (*cmd)->args);
  exit(1);
}
