...
void sksh_print(ivector *outer, ivector *inner, ivector *cont)
{
  ...
  r0 = 0;
  while (r0 < ilen && iv_elem(inner, r0) == iv_elem(outer, r0))
    r0++;
               ...
               int maxrows, int maxcols)
{
  ...
  fr1 = 0;
  while (fr1 < len1 && iv_elem(sh1, fr1) == maxcols)
    fr1++;
  fc2 = (len2 == maxrows && len2 > 0) ? iv_elem(sh2, len2 - 1) : 0;
  fr2 = 0;
  while (fr2 < len2 && iv_elem(sh2, fr2) == maxcols)
    fr2++;

  /* Find # boxes after removing full rows and columns. */
  sz1 = (fr1 - len1) * fc1;
  for (r = len1 - 1; r >= fr1; r--)
    sz1 += iv_elem(sh1, r);
  sz2 = (fr2 - len2) * fc2;
  for (r = len2 - 1; r >= fr2; r--)
    sz2 += iv_elem(sh2, r);
  ...
  if (sz1 > sz2)
    {
      ivector *tiv;
      int ti;
      tiv = sh1; sh1 = sh2; sh2 = tiv;
      ti = len1; len1 = len2; len2 = ti;
      ti = sh10; sh10 = sh20; sh20 = ti;
      ti = fc1; fc1 = fc2; fc2 = ti;
      ti = fr1; fr1 = fr2; fr2 = ti;
    }
  ...
  if (out == NULL)
    return -1;
  for (r = 0; r < len1 - fr1; r++)
    iv_elem(out, r) = iv_elem(sh1, fr1 + r) - fc1;
  ...
  for (r = 0; r < fr1; r++)
    iv_elem(cont, r) = maxcols;
  for (r = 0; r < len2; r++)
    iv_elem(cont, fr1 + r) = iv_elem(sh2, r) + fc1;
  for (r = len2 + fr1; r < clen; r++)
    iv_elem(cont, r) = fc1;
		 ...
		 int rows, int level)
{
  ...
  /* Empty result? */
  memset(ss, 0, sizeof(skew_shape));
  if (part_length(sh1) > rows || part_length(sh2) > rows)
    return 0;
  ...
  if (nsh1 == NULL)
    return -1;
  for (i = 0; i < rows - d; i++)
    iv_elem(nsh1, i) = part_entry(sh1, d + i) - sh1d + level;
  for (i = 0; i < d; i++)
    iv_elem(nsh1, rows - d + i) = part_entry(sh1, i) - sh1d;
  nsh2 = iv_new(rows);
  if (nsh2 == NULL)
    {
      iv_free(nsh1);
  ...
  for (i = 0; i < d; i++)
    iv_elem(nsh2, i) = part_entry(sh2, rows - d + i) + sh1d;
  for (i = 0; i < rows - d; i++)
    iv_elem(nsh2, d + i) = part_entry(sh2, i) + sh1d - level;
	       ...
	       ivector *content, int maxrows)
{
  ...
  int row_bound, row_first, row_span, ilen, clen, slen, r, c, c1, c2,
    r0t, r0b, r1t, r1b, r2t, r2b, full_cols, cont_size, comp_size;
  partial_shape ps;

  /* Handle case in other function. */
  if (inner == NULL)
    return optim_mult(ss, outer, content, maxrows, -1);

  /* DEBUG: Check valid input. */
  claim(part_valid(outer));
  claim(part_valid(inner));
  if (content != NULL)
    claim(part_valid(content));

  /* Indicate empty result. */
  memset(ss, 0, sizeof(skew_shape));
  if (part_leq(inner, outer) == 0)
    return 0;
  ...
  r2t = row_first;
  r2b = row_first;
  while (r2b < row_bound && c2 <= iv_elem(outer, r2b))
    r2b++;
...
int optim_coef(skew_shape *ss, ivector *out, ivector *sh1, ivector *sh2)
{
  ...
  N = part_length(out);
  if (N < iv_length(sh1) && iv_elem(sh1, N) > 0)
    return 0;
  if (N < iv_length(sh2) && iv_elem(sh2, N) > 0)
    return 0;
  if (N == 0)
    {
      ss->sign = 1;
  ...
  for (Nla = N; Nla > iv_length(sh1); Nla--)
    iv_elem(la, Nla-1) = 0;
  for (; Nla > 0 && iv_elem(sh1, Nla-1) == 0; Nla--)
    iv_elem(la, Nla-1) = 0;
  for (r = Nla-1; r >= 0; r--)
    {
  ...
  for (Nmu = N; Nmu > iv_length(sh2); Nmu--)
    iv_elem(mu, Nmu-1) = 0;
  for (; Nmu > 0 && iv_elem(sh2, Nmu-1) == 0; Nmu--)
    iv_elem(mu, Nmu-1) = 0;
  for (r = Nmu-1; r >= 0; r--)
    {
